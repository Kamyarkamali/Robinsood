...
function DateSelector({
  ...
  const getWeeks = () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const label = isRTL
        ? `${toPersianDigits(weekStart.getDate())} - ${toPersianDigits(weekEnd.getDate())} ${getMonthName(weekStart.getMonth())}`
...
function WeeklyReportPanel() {
  ...
  const filteredData = weeklyReport.filter((row) => {
    return true;
  });

  const getStatistics = () => {
    if (tab === "pnl") {
      const totalWin = filteredData.reduce(
        (sum, row) => sum + (row.amount * row.winPercent) / 100,
        0,
      );
      const totalLoss = filteredData.reduce(
        (sum, row) => sum + (row.amount * row.losePercent) / 100,
        0,
      );
      const totalProfit = totalWin - totalLoss;

      return {
        title: isRTL ? "مجموع سود/ضرر" : "Total P&L",
        value: totalProfit.toFixed(2),
        isPositive: totalProfit >= 0,
      };
    } else {
      const avgWinPercent =
        filteredData.reduce((sum, row) => sum + row.winPercent, 0) /
        filteredData.length;
      const avgLossPercent =
        filteredData.reduce((sum, row) => sum + row.losePercent, 0) /
        filteredData.length;
      const riskReward = avgWinPercent / avgLossPercent;

      return {
        title: isRTL ? "نسبت ریسک به ریوارد" : "Risk/Reward Ratio",
        value: riskReward.toFixed(2),
        isPositive: riskReward >= 1,
      };
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col mt-10 gap-4 sm:gap-5 flex-1 justify-between overflow-y-auto max-h-[400px] pr-1">
