      ...
      >
        ...
        <div className="px-4 md:px-6 pb-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                return (
                  <button
                    key={lang.code}
                    onClick={() => changeLang(lang.code as Lang)}
                    className={`
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                { id: "light", icon: Sun, label: isFa ? "روشن" : "Light" },
                { id: "dark", icon: Moon, label: isFa ? "تاریک" : "Dark" },
              ].map((theme) => {
                ...
                return (
                  <button
                    key={theme.id}
                    onClick={() => changeTheme(theme.id as "dark" | "light")}
                    className={`
                      relative flex items-center justify-center gap-2
                      p-3 rounded-xl
                      transition-all duration-300
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-white/60 text-xs font-medium">
                ...
                <span>{isFa ? "آواتار" : "Avatar"}</span>
              </div>
              <button
                onClick={() => setShowAvatarModal(true)}
                className="
                  text-xs text-violet-400 hover:text-violet-300
            ...
            <div className="flex items-center gap-4 p-3 rounded-xl bg-white/5 border border-white/10">
              <div className="w-14 h-14 rounded-full overflow-hidden border-2 border-violet-500/30">
                <img
                  src={user.avatar}
                  alt="avatar"
              ...
              <div>
                ...
                <p className="text-xs text-white/40">
                  {isFa ? "برای تغییر کلیک کنید" : "Click to change"}
                </p>
              </div>
            </div>
          </div>
          ...
          >
            <motion.div
              ...
              className="
                w-full max-w-md
                rounded-2xl
                bg-linear-to-b from-gray-900/95 to-black/95
                backdrop-blur-xl
                border border-white/10
                shadow-2xl
                overflow-hidden
              ...
              onClick={(e) => e.stopPropagation()}
            >
              <div className="sticky top-0 z-10 px-4 pt-4 pb-3 bg-linear-to-b from-gray-900/95 to-transparent">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-bold text-white">
                    {isFa ? "انتخاب آواتار" : "Select Avatar"}
                  </h3>
                  <button
                    onClick={() => setShowAvatarModal(false)}
                    className="
              ...
              <div className="px-4 pb-6">
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                  {avatarData.map((item, index) => {
                    ...
                    const displayName = isFa ? item.fa : item.en;
                    const isHovered = hoveredAvatar === item.id;
                    ...
                    return (
                      <motion.div
                        ...
                        onMouseEnter={() => setHoveredAvatar(item.id)}
